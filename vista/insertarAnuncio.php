...
if($_SERVER['REQUEST_METHOD']=='POST'){
   ...
   /*  if($arrFotos['type'][0] != 'image/jpeg' && $arrFotos['type'][0] != 'image/webp' && 
        $arrFotos['type'][0] != 'image/png' && $arrFotos['type'][0] != 'image/jpg'){
            $errorFoto = "La foto no tiene el formato admitido, debe ser jpg, webp, png o jpg";
    }else{
        $fotoAnuncio = generarNombreArchivo(htmlspecialchars($arrFotos['name'][0]));
        if(!move_uploaded_file($arrFotos['tmp_name'][0], "../fotosAnuncio/$fotoAnuncio")){
            die("No mete las fotos principales");
        }
    ...
    for ($i=0; $i < $arrLength; $i++) { 
        ...
        $arrFotos['type'][$i] != 'image/png' && $arrFotos['type'][$i] != 'image/jpg'){
            ...
            if(!move_uploaded_file($arrFotos['tmp_name'][$i], '../fotosTablaFotos/'.$foto)){
                die("no mete las fotos de tabla fotos");
            }

            if($i==0){
                $anuncio->setFoto($foto);
                $idAnuncio = $anuncioDAO->insert($anuncio, $idUsuario);
                copy('../fotosTablaFotos/'.$foto, '../fotosAnuncio/'.$foto);
            }
        ...
        <article>
            <section class="contenido">
                <div class="container">
                    ...
                    <form action="insertarAnuncio.php" method="post" enctype="multipart/form-data" id="formulario">
                        <div class="form-row">
                        ...
                        <div class="form-row">
                            <div class="input-data" style="width: 45%;">
                                <input type="file" name="foto[]"  accept="image/jpeg, image/jpg, image/webp, image/png" required multiple>
