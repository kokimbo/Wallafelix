...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    $arrLength = count($arrFotos['name']);

    if($arrFotos['type'][$arrLength-1] != 'image/jpeg' && $arrFotos['type'][$arrLength-1] != 'image/webp' && 
        $arrFotos['type'][$arrLength-1] != 'image/png' && $arrFotos['type'][$arrLength-1] != 'image/jpg'){
            $errorFoto = "La foto no tiene el formato admitido, debe ser jpg, webp, png o jpg";
    }else{
        $fotoAnuncio = generarNombreArchivo(htmlspecialchars($arrFotos['name'][$arrLength-1]));
        if(!move_uploaded_file($arrFotos['tmp_name'][$arrLength-1], "../fotosAnuncio/$fotoAnuncio")){
            die();
        }
    }
    ...
    $anuncio->setFoto($fotoAnuncio);

    $idAnuncio = $anuncioDAO->insert($anuncio, $idUsuario);

    for ($i=0; $i < $arrLength-1; $i++) { 
        if($arrFotos['type'][$i] != 'image/jpeg' && $arrFotos['type'][$i] != 'image/webp' && 
        $arrFotos['type'][$i] != 'image/png' && $arrFotos['type'][$i] != 'image/jpg'){
            $errorFoto = "La foto no tiene el formato admitido, debe ser jpg, webp, png o jpg";
        }else{
            $foto = generarNombreArchivo(htmlspecialchars($arrFotos['name'][$i]));

            while(file_exists("../fotosTablaFotos/$foto")){
                $foto = generarNombreArchivo(htmlspecialchars($arrFotos['name'][$i]));
            }
            
            if(!move_uploaded_file($arrFotos['tmp_name'][$i], '../fotosTablaFotos/'.$foto)){
                die();
            }
            $fotoDAO->insert($foto, $idAnuncio);
        }
    }
}
...
<body>
    <main>
        ...
        <section class="leyenda">
            <div>
                <span><strong>¡Hola <?= $_SESSION['nombre'] ?>! Bienvenido a tus anuncios,</strong></span>
                <span>aqui podras modificar o borrar los anuncios que ya tengas subidos</span>
            </div>
        </section>

        <article>
            <section style="" class="contenido">
