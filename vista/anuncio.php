...
$conn = $connexionDB->getConnexion();
    ...
    if($_SERVER['REQUEST_METHOD']=='GET'){
        ...
        $arrFotos = array();
        $arrFotosArchivo = array();


        if($anuncioSeleccionado = $anuncioDAO->getById($idAnuncio)){
            $titulo = $anuncioSeleccionado->getTitulo();
            $precio = $anuncioSeleccionado->getPrecio();
            $descripcion = $anuncioSeleccionado->getDescripcion();
            $foto = $anuncioSeleccionado->getFoto();
            $arrFotos = $fotoDAO->getAllByIdAnuncio($idAnuncio);
            
            foreach ($arrFotos as $fotos) {
              $arrFotosArchivo[] = $fotos->getFotoAnuncio();
            }
            /* array_unshift($arrFotosArchivo, $foto); */

            $idUsuario = $anuncioSeleccionado->getIdUsuario();
            $usuario = $usuarioDAO->getById($idUsuario);

            $tituloTrz1 = substr($titulo, 0, strpos($titulo, ' '));
            $tituloTrz2 = substr($titulo, strpos($titulo, ' '), strlen($titulo));
        }else{
...
<head>
    ...
    <link rel="stylesheet" href="../estilos/anuncio.css">
...
<body>
    ...
    <div class="card">
      ...
      <div class="card__body">
        ...
        <div class="half">
          ...
          <span class="stock"><i class="fa fa-pen"></i>Todavia disponible</span>
          <div class="reviews">
            <ul class="stars">
            ...
            <span><?= rand(1,255) ?> views</span>
            <div class="recommend">
                <a href="../index.php"><p>Volver atras</p></a>
            </div>
          </div>
        </div>
      </div>
      <div class="card__footer">
        <div class="recommend">
          <p>Anuncio subido por</p>
          <h3><?= $usuario->getNombre() ?> con telefono: <?= $usuario->getTelefono() ?></h3>
        </div>
        <div class="action">
  ...
  <script>
    ...
    let pictures = <?php echo json_encode($arrFotosArchivo);?>;
    img.src = "../fotosTablaFotos/"+pictures[0];
    let position = 0;

    const moveRight = () => {
        if (position >= pictures.length - 1) {
            position = 0
            img.src = "../fotosTablaFotos/"+pictures[position];
            return;
        }
        img.src = "../fotosTablaFotos/"+pictures[position + 1];
        position++;
    }
